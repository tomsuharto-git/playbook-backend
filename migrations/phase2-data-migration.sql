...
BEGIN
  ...
  event_extract AS (
    SELECT
      ...
      COALESCE(generated_at, created_at, NOW()) as created_at
    FROM event_data
...
BEGIN
  ...
  WITH brief_narratives AS (
    SELECT
      ...
      COALESCE(db.generated_at::date, db.created_at::date, CURRENT_DATE) as date,
      COALESCE(
        'Daily Brief: ' || to_char(COALESCE(db.generated_at, db.created_at), 'Mon DD'),
        'Daily Brief'
      ) as headline,
      ARRAY[
...
BEGIN
  ...
  RAISE NOTICE '2. Test the new central processor';
  RAISE NOTICE '3. Update your email and vault watchers';
  RAISE NOTICE '';
